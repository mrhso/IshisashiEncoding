...
GB 18030-2000 为原本，在 Windows 代码页为 CP 54936。需要注意的是，这个 GB 18030-2000 是 2000 年 12 月修订的版本，而非 2000 年 3 月发布的初版。

GB 18030-2005 在 GB 18030-2000 的基础上互换了「ḿ」和「」的映射。

GB 18030-2022 在 GB 18030-2005 的基础上以互换映射和删除字形的手法将 PUA 遗留全部消除。
...
|四字节|0x81\~0xFE|0x30\~0x39|0x81\~0xFE|0x30\~0x39|1587600|实际使用 1089964 个。|
...
## 初版 GB 18030-2000 与报批稿
初版有映射到 U+20AC 的单字节欧元符号 0x80 与映射到 U+E76C 的双字节欧元符号 0xA2E3，修订版只有映射到 U+20AC 的双字节欧元符号 0xA2E3。

初版调整了 78 个 GBK-PUA 字的映射，但不在附录 A（以及附录 B、附录 C？）调整（这使附录 A 的内容与 GBK 一致），而是在附录 E 给出——附录 E 中实际列出了 79 个，其中漏调整的是 0xA989「〾」；修订版调整了 80 个，除了补上遗漏的 0xA989「〾」外，还追加调整了 0xFE5E「⺗」。

就四字节 BMP 部分而言，初版跳过 U+0080 与调整过的 GBK-PUA 字之 PUA 码点，不跳过代理区；修订版跳过代理区，不跳过其余全部字符。

GB 18030-2000 报批稿的映射表与初版相同，却没有编写附录 E。从附录 B 与附录 C 使用 PUA 的事实可以推测，报批稿的附录 A 与初版一样，也没有明示调整，只是忘了编写附录 E 而已。
